/*
  ...
  +----------------------------------------------------------------------+
  | Author:Sakura Wong   cloud976@gmail.com                              |
  +----------------------------------------------------------------------+
*/
...
const zend_function_entry foolsock_methods[] = {
    ...
    ZEND_ME(foolsock, test, NULL, ZEND_ACC_PUBLIC)
...
PHP_METHOD(foolsock, readLine)
{
    ...
    if (size > 0) {
        response_buf = emalloc(size + 1);
        response_buf = php_stream_get_line(f_obj->stream, response_buf, size, &line_length);
        if (line_length == 0 || response_buf == NULL) {
            if (errno == EAGAIN || errno == EINPROGRESS) {
                RETURN_TRUE;
            }
            RETURN_FALSE;
        ...
        response_buf = php_stream_gets(f_obj->stream, NULL, size);
        if (response_buf == NULL) {
        	RETURN_FALSE;
        }
        RETURN_STRING(response_buf, 0);
    }
}
/*}}}*/
...
PHP_METHOD(foolsock, test)
{
    ...
    if (f_obj->stream != NULL) {
    	avail = f_obj->stream->writepos - f_obj->stream->readpos;
   		php_printf("%d|%d|%d|%d",avail,php_stream_eof(f_obj->stream),f_obj->stream->writepos,f_obj->stream->readpos);
    }
...
PHP_MINFO_FUNCTION(foolsock)
{
    ...
    php_info_print_table_header(2, "foolsock support", PHP_FOOLSOCK_VERSION);
